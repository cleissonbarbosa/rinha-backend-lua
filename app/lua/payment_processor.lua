...
local function process_with_processor(payment_data, processor_type)
    ...
    if res.status == 200 then
        -- proceed to record below with processing_iso
    elseif res.status == 409 then
        -- Duplicate correlationId: PP likely already processed previously.
        -- Fetch details to get the original requestedAt and align our accounting window.
        ...
        local okd, data = pcall(cjson.decode, details_res.body)
        if not okd or not data or not data.requestedAt then
            return false, "invalid details payload from " .. processor_type
        end
        -- Override processing_iso with the original requestedAt from PP
